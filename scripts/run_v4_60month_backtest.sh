#!/bin/bash
# V4 129维 60个月回测：构建预测缓存 → 并行回测（nohup 脱离会话，断点续跑，进度微信推送）
...
if [ "${SKIP_PROGRESS:-0}" != "1" ]; then
    (
        ...
        while true; do
            ...
            pct=$(( DONE * 100 / TOTAL ))
            notify "⏳ V4 60月回测进度: 已构建 $DONE/$TOTAL 个月 ($pct%)，失败月份 $ERR。（断点续跑，继续中）"
...
notify "🚀 V4 129维 60个月回测启动（2020-09~2026-06，与旧体系同口径）。后端预测缓存构建+并行回测，每15分钟推进度，完成后推最终结果。"

# ── 阶段1: 构建 60 个月 V4 预测缓存（多进程24/断点续跑）──
log "阶段1: 构建 60 个月 V4 预测缓存 -> $OUT"
START_T=$(date +%s)
...
log "阶段2 回测: exit=$RC2"
if [ $RC2 -eq 0 ]; then
    notify "✅ V4 129维 60个月回测完成。结果见 output/backtest_v2/，对比旧121维评估收益是否不劣化。"
else
    notify "⚠️ V4 60月回测 exit=$RC2（部分配置可能完成，见回测 CSV/JSON）。"
fi
log "═══ V4 60月回测链结束 ═══"
