...
系统在交易日 **20:55** 由服务器 cron 自动触发，执行以下完整流程：

```
20:55:00  启动
    ↓
增量同步 → baostock 单进程顺序拉取全量 5207 只股票当日数据
           （因 baostock 不支持多进程并发，故使用单进程 + 200ms 间隔）
           预计耗时 15~20 分钟
    ↓
基础股票池 → 板块/ST/次新/价格四重过滤
             约 2800 只
    ↓
7 策略独立选股 → 每策略从基础池中筛选 → 按分数取前 5
    ↓
DeepSeek LLM 综合研判 → 大盘/个股行情/基本面/新闻/股吧舆情
    ↓
21:10 左右  WxPusher 推送到微信（仅一条综合研判报告）
21:15        WorkBuddy 自动化推送监控报告
```

> **⚠️ 重要说明**：baostock 的 `SocketUtil` 为单例模式，其服务器端对同一用户有并发连接限制。多进程并行会导致 login 死锁、logout failed 和数据丢失。因此增量同步和回填均**强制使用单进程顺序拉取**，每只股票间隔 200ms 防封。

### 日志中的时间信息

```log
[20:55:01] Sequoia-X V2 启动
[20:55:05] 需要更新 4600 只股票，单进程顺序拉取（每只间隔200ms）...
[21:09:50] 增量同步完成: 写入 4200 条数据（遍历 4600 只）耗时 885秒   ← 含完成时间和用时
[21:10:07] 基础股票池最终: 2830 只
[21:10:12] 执行策略：均量线突破 → 选出 5 只
...
[21:12:30] Sequoia-X V2 运行完成（总耗时 809 秒）
```
