...
> 版本: v1.3 | 日期: 2026-07-22 | 状态: 已确认 (v1.2全部+v1.2.1数据源+v1.3策略增强+日报隔离)
...
| LLM 模拟盘 | ✅ 运行中 |
| 双策略汇总推送 | ✅ 运行中 |
| LSTM 首笔交易 | 📅 7/23 (周四) — 周三荐股→周四T+1买入 |

---

## 十七、v1.3 策略增强 + 日报隔离修复 (2026-07-22)

### 17.1 策略 A: LSTM 预测作为卖出因子

新增两条 rules.py 规则:
- `_check_lstm_factor(symbol)`: 读取 daily.py 保存的预测缓存 JSON
  - 预测 > +2% → LSTM_BUY_BONUS=-30(延迟卖出)
  - 预测 < 0%  → LSTM_SELL_PENALTY=+20(加速卖出)
- `_check_min_hold(hold_days, total_score)`: 持有 < 5 天强制阻止卖出(硬止损 S1 例外)

数据流: `daily.py 预测 → JSON缓存 → SimEngine evaluate_exit 读取`

### 17.2 策略 B: 扩大持仓释放 Rank IC (待回测)

| 参数 | 旧 | 新 | 依据 |
|------|:---:|:---:|------|
| max_positions | 10 | 30 | 大数定律放大 IC 0.16 |
| top_n_buy_per_day | 2 | 5 | 6天建满仓 |
| per_stock_budget | 5万 | 1.6万 | 30×1.6万≈48万 |
| min_pred_return | 1% | 0% | 纯靠排序 |

> ⚠️ 生产参数已回滚至旧值, 待回测验证后启用。

### 17.3 回测多配置对比框架

`backtest_compare.py`:
- Phase 1: 全周期(615天)每日预测缓存 (~16h)
- Phase 2: 4组配置回放 (~10min): baseline / A / B / A+B
- 回测引擎现已使用完整 `rules.py evaluate_exit` + 预测缓存供 LSTM 因子读取

### 17.4 日报推送隔离修复

- **问题**: `SimEngine.run_daily()` 每次调用都推送通用"Sequoia-X模拟盘日报"
  LSTM SimEngine 也推送 → 两份同名报告内容不同 → 用户误认 LSTM 混用 LLM 持仓
- **修复**: `run_daily(push_report=True)` 参数, LSTM 传 `False`
  推送清单: LLM通用日报 + LSTM独立日报 + 策略汇总, 标题互不混淆

### 17.5 首次交易时间线

- 7/21(周二): 手动测试, 产生不应存在的信号
- 7/22(周三): 管线正式运行, LSTM 荐股 601567/600415, 空仓
- 7/23(周四): T+1 首次买入 601567/600415
