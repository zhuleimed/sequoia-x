# Sequoia-X V2 体系完整操作指南（最终版 v5.3，2026-08-17）
...
>
> **版本要点（v5.3，2026-08-17）**：
> - **V2 模拟盘月末清仓模式 A 定稿（两处口径修正）**：
>   ① 清仓时点：月末**最后交易日**（akshare 交易日历判定，非固定 30/31 日）以**当日收盘价**卖出全部持仓
>   ② 买入时点：重训日（每月 1 日 03:00）若为交易日 → **当天晚上以当日开盘价**买入新 TOP10
>      （信号凌晨已就绪）；非交易日顺延首个交易日（allow_same_day 机制，与回测"次月首日开盘买入"口径一致）
> - **A/B 模式回测对比**（70 个月，M4+TOP10）：模式 A（月末清仓）+988.1% vs 模式 B（不清仓补空位）+953.7%——A 略优 3.5%，夏普/回撤/胜率几乎相同；**决策：选 A**（详见 BACKTEST_PLAN §29）
> - v5.2 要点：
...
| **月末清仓** | **模式 A：月末最后交易日（交易日历判定）以收盘价清仓 → 重训日当天/顺延首日满仓新 TOP10**（A/B 实证 A 略优 3.5%） | 2026-08-17 |
...
综合最好的 10 只——**回测证明融合比单独用任何一个都赚钱**（最优配置年化 52.1%（70月））。
...
- **月末换仓**（2026-08-17 定稿）：月末**最后交易日**（交易日历判定，非固定 30/31 日）以当日
  **收盘价**卖出全部持仓；下月 1 日 03:00 重训选出的新 TOP10，在重训日（若为交易日）当天晚上
  以**当日开盘价**买入，非交易日则顺延首个交易日——**每月用新模型选的新股票重新满仓，
  与回测口径完全一致**（回测实证：月末清仓模式收益略高于让幸存者继续持有）
...
- **最终结果（旧 121 维基线）**（70 个月回测，2020-09~2026-06）：T2+T4 融合、M4 风控、TOP_N=10 → **总收益 +944.8%、年化 52.1%、夏普 2.28、回撤 -18.2%、胜率 70%**（见 BACKTEST_PLAN §27.2）。
  - ⚠️ **口径修正**：早年文档的"年化 89.5%、夏普 3.35"是 **11 个月**回测的总收益/夏普（BACKTEST_PLAN §"最终最佳配置"），被误记为"70 个月年化"。权威 70 个月旧基线结果为 52.1%/2.28。
- **V4（129 维）对比（2026-08-21 实测，去前视后 summary_all.csv 08-21 18:31）**：70 月同口径下 M4_TOP10 旧 121维(+944.8%/52.1%/2.28) vs **V4 129维去前视(+2077%/年化73.4%/3.06)**——**显著提升**（终值 21.8× vs 10.4×，回撤 -13.4% vs -18.2%）。最优 M4_15：年化 68.6%/夏普 3.22。
  - **🔑 去前视结论（2026-08-21）**：龙虎榜/涨停已改为次一交易日生效（消除月末采样前视，commit a7e32a5）。**去前视版年化 73.4% vs 昨前视版 73.8%，几乎不变** → **昨晚提升是真实的，与龙虎榜/涨停前视无关**（特征重要性证实: 龙虎榜/涨停/xdxr gain=0.000%，模型从不看这些维度）。V4 提升来自 finance 16维扩展 + 换同花顺更全数据源，而非情绪维度。
  - ⚠️ **绝对年化（~73%）偏高，属 super-alpha 水平，需独立复核**（回测引擎对暴涨股敏感易含幸存者偏差，不宜直接当实盘预期；相对验证的是 V4 vs 旧体系选股能力提升）。
- **✅ 稳健性复核（2026-08-21，`scripts/v4_robustness_check.py` + `data/v4_robustness_check.json`）**：去最佳月（2026-03 +47.67%）→ 月化年化 55.2%→45.9%；去 3 月→34.5%；去 6 月→24.3%。单笔最大盈利 Top10 全部集中在 **2026-03~06 真实电力板块暴涨**（600396 华电辽能 +56% 等，2026 年 4-6 月 +540%）。**结论**：
  - **相对提升真实**：V4 vs 旧 121维，去噪后仍略优（V4+2077% vs 旧+944.8%）。
  - **绝对收益 73.4%/21.8× 不可当实盘预期**：实质是恰好押中 2026 年 3-6 月电力板块暴涨，含幸存者偏差/选股集中度，非前视、非普遍规律。
- **🔑 8/31 切换决策（用户拍板，2026-08-21）**：**切 V4**。理由：相对选股能力确定提升（口径一致跨牛熊）、彻底摆脱 baostock 限流（换同花顺）、回撤更低。但**实盘预期年化按下调**——不能用 73% 当目标，按旧体系 52% 档或更低做预期管理。
...
**融合**：T2+T4 Rank 融合选股（回测最优 M4+TOP_N=10，年化 52.1% 夏普 2.28（70月））。
...
- **月末清仓（模式 A，2026-08-17 定稿）**：月末最后交易日（akshare 交易日历判定，非固定 30/31 日）
  以**当日收盘价**卖出全部持仓（liquidate_all_at_close）→ 重训日=交易日当天晚上以**当日开盘价**
  买入新 TOP10（非交易日顺延）→ 满仓（实现：v2_simulation_daily.py 月末分支 + SimEngine allow_same_day）
- 非重训日唯一买卖 = 风控卖出（无新买入）
...
| **月末清仓** | **月末最后交易日以当日收盘价卖出全部持仓 → 重训日当天/顺延首日满仓新 TOP10（模式 A，2026-08-17 定稿）** |
...
| 逐月 IC 分析 | 70 个月全貌（2020-09~2026-06；早年"60 个月"为规划表述，实际 70） | 2025 后 IC 衰减（风格切换） |

**最终最优配置（70 个月，2020-09~2026-06）**：T2+T4 Rank 融合，M4 风控，TOP_N=10，
总收益 +944.8%、年化 52.1%，夏普 2.28，回撤 -18.2%，胜率 70%（2026-08-21 修正口径，见 §300 注）。
**核心结论**：2026 风格切换后（y2 均值 -10.8%），滚动窗口 +
