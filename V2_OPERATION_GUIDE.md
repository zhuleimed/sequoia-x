...
> - **月末清仓模式 A 定稿**：V2 模拟盘月末最后交易日标记全部持仓"月末清仓（换仓）" → 下月首日开盘先卖后买 → 满仓新 TOP10（与回测 M4+TOP_N=10 口径完全一致）
...
| **月末清仓** | **模式 A：月末最后交易日标记全部持仓清仓 → 下月初满仓新 TOP10**（A/B 对比实证 A 略优 3.5%，与回测口径一致） | 2026-08-17 |
...
- **月末换仓**（2026-08-17 定稿）：每月最后交易日把全部持仓标记"月末清仓（换仓）"，
  下月 1 日重训选出的新 TOP10 在下月首个交易日开盘满仓买入——**每月用新模型选的新股票重新满仓，
  与回测"月末清仓 + 月初换仓"口径完全一致**（回测实证：月末清仓模式收益略高于让幸存者继续持有）
...
- **月末清仓（模式 A，2026-08-17 定稿）**：月末最后交易日标记全部持仓"月末清仓（换仓）" →
  下月首日开盘先卖后买 → 满仓新 TOP10（与回测口径一致；实现：v2_simulation_daily.py 月末分支）
- 非重训日唯一买卖 = 风控卖出（无新买入）
...
| **月末清仓** | **月末最后交易日标记全部持仓"月末清仓（换仓）" → 下月首日先卖后买（模式 A，2026-08-17 定稿）** |
