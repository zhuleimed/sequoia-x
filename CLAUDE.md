...
**70 个月扩展回测已完成**（2020-09~2026-06，全文口径统一为 70 个月；早年文档"60 个月"为扩展早期规划表述，实际贯穿回测为 70 个月），V2/V3 体系稳定运行中。
...
4. **本节"当前状态"** ← 70 个月（2020-09~2026-06）扩展回测流水线
5. **`docs/2026-09_量化研究方向指导手册.md`**（+同名 `.docx`）← **长期（living）研究方向总览地图**：机构九方向总览 + 每个由浅入深路线/门槛。**今后凡开展/证伪一个方向，都回该 .md 对应方向"补记"填四行(方向/数据口径/门槛结果/是否换血)，改完跑 `python scripts/md2docx_guide.py` 重生成 .docx**。结构含 OOS 讲解、术语表(OOS/IC/IR/夏普/过拟合)、个人筹码数据盘点。
...
### amount 单位铁律（2026-09-18 事故后新增）

**stock_daily.amount 的约定单位是「元」**，且必须满足自证式校验：**`amount / volume ≈ close`**（隐含价应落在当日价格区间内）。

- 事故：`sync.py` 的**腾讯实时行情降级路径**把接口的成交额（单位**万元**）直接写进 amount，
  当日成交额小 10000 倍；同一路径还把 `turnover` 写死 0.0。实测 08-04~09-17 共 **118,530 行**受影响
  （波及 022 的 turnover_rate/成交额/VWAP 类因子与 021 因子库的换手类因子）。
  同期还有 1,016 行科创板（688 系，08-10~08-21）的 **volume 大 100 倍**。
  已在 `scripts/fix_amount_unit_20260918.py` 修复（原值备份在 `data/backup_amount_unit_fix_20260918/`）。
- **降级路径写 NULL（NaN），不要写 0.0** —— 0.0 读作"真实值为 0"，NULL 才是"未知"。
- 任何新增/修改数据源的路径：先确认 amount、turnover、volume 的单位再入库，
  并用 `amount/volume ≈ close` 做一次抽样自证（这一条能一次抓出所有单位类错误）。
...
## 重要：时长分析必须用系统时间（2026-08-20 用户补充）

**分析"还要多久/耗时"必须以 `date` 系统时间为基准，禁止用 CPU 用时（`ps` 的 TIME/etime%CPU）推算**：
- 本项目大量任务**多进程/多 worker**（缓存重建 16 worker、LSTM 多核），`ps` 的 CPU 用时是该进程**累计 CPU 时间**（会被多进程叠加放大，且与墙钟严重偏离），作时长分析**不可靠**。
- **正确做法**：起止各执行一次 `date` 取墙钟时间差；或 `ps -eo etime`（墙钟已运行时长，非 CPU 累计）。
- 违反示例：误用"%CPU 高"当"还在跑"、用 CPU 累计时间当实际耗时，导致 ETA 偏差。
...
**V4 vs V3 明确边界（2026-08-20 用户决策，勿混）**：
- **V4（9月起执行）** = V2 数据/特征升级（129维, feature_version=4）+ **保留现有 T2分布空仓** 风控。
  **不接入指数择时开关**（`monthly_engine.py` 的 top100均值<-5%→空仓 机制维持）。
- **V3** = 有指数择时开关（设计预设，`[ ]` 从未落地）；**不落地、不做 V3 回测**。
- 即：**指数择时开关仅存在于 V3 设计，不进入实际运行的 V4**。择时仅用于独立 ETF 模拟盘（`sim_etf_timing.py`，与个股模拟盘无关）。
